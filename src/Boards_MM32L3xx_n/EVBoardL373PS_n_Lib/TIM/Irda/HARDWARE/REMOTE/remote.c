...
* @version V1.3.9
* @date    28/08/2019
* @brief   This file provides all the remote firmware functions.
******************************************************************************
* @copy
*
...
void Remote_Init(void)
{
    ...
    RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOC | RCC_APB2Periph_AFIO, ENABLE); //使能GPIOC&AFIO时钟
    RCC_APB1PeriphClockCmd(RCC_APB1Periph_TIM3, ENABLE);	//TIM2 时钟使能

    GPIO_PinRemapConfig(GPIO_FullRemap_TIM3, ENABLE);//I2C1重映射


    GPIO_InitStructure.GPIO_Pin = GPIO_Pin_9;				 //PA1 输入
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPD; 		//下拉输入
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
    GPIO_Init(GPIOC, &GPIO_InitStructure);
    GPIO_SetBits(GPIOC, GPIO_Pin_9);	//初始化GPIOA1
